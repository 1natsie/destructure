...
import { createGrowingBuffer, getStringCharCodes } from "../utils/utils.js";
const dvMethodMap = {
...
const getBigInt = (value, segmentCount) => {
    if (typeof value === "bigint")
        return value;
    if (typeof value === "number")
        return BigInt(value);
    if (!Array.isArray(value))
        throw new Error("value must be a number, bigint, or array.");
        ...
        switch (schema.schemaType) {
            ...
            case SchemaType.Simple: {
                if (schema.isArray && !Array.isArray(payload)) {
                    throw new TypeError(`Invalid data. Data must be an array for schema: ${schema.base}[${schema.count}].`);
                }
                switch (schema.base) {
                    case "char": {
                        const _payload = Array.from(payload).flatMap((p) => getStringCharCodes(p, (c) => {
                            if (c > 255)
                        ...
                        if (schema.count === -1) {
                            ...
                            if (_payload.length !== schema.count) {
                                throw new Error(`Element count mismatch between schema and data. Expected ${schema.count} elements, but received ${_payload.length}.`);
                            }
                            buffer.ensureCapacity(schema.byteLength * schema.count);
                            count = schema.count;
                        }
                        for (let i = 0; i < count; i++)
                            buffer.writeOne(_payload[i]);
                        break;
                    }
                    case "u8":
                    case "u16":
            ...
            case SchemaType.Custom: {
                if ("encodeInto" in schema.handler && schema.handler.encodeInto != null) {
                    ...
                    buffer.write(schema.handler.encode(payload));
                break;
            }
            default: {
