...
import type { SchemaType as _SchemaType, bitpackSchemaKey, compiledSchemaKey, customSchemaKey, optionalSchemaKey } from "./schema.ts";
...
type ObjectSchema = {
    [x: string]: Schema | CompiledSchema;
};
type TupleSchema = SchemaOrCompiledSchema[];
type NullSchema = null;
type OptionalSchema<T extends Schema = Schema> = {
    [optionalSchemaKey]: T | true;
};
type BitpackSchema<T extends number = number> = {
    [bitpackSchemaKey]: T | true;
};
...
interface CustomSchemaHandler<InputType = any, OutputType extends any = InputType> {
    ...
    }, offset: number) => DecodeResult<OutputType>;
    size: () => SizeRange;
}
...
type CompiledSchema<T extends Schema = Schema> = CompiledNullSchema<T> | CompiledSimpleSchema<T> | CompiledObjectSchema<T> | CompiledTupleSchema<T> | CompiledArraySchema<T> | CompiledBitpackSchema<T> | CompiledOptionalSchema<T> | CompiledCustomSchema<T>;
declare namespace Data {
    ...
    type DecodeInputSimple<T extends SimpleSchema> = T extends SimpleSchemaBase ? _SimpleSchemaTypeMap["Input"][T] : T extends `${infer PT extends SimpleSchemaBase}[${number | ""}]` ? _SimpleSchemaTypeMap["Input"][PT][] : never;
    type DecodeOutputSimple<T extends SimpleSchema> = T extends SimpleSchemaBase ? _SimpleSchemaTypeMap["Output"][T] : T extends `${infer PT extends SimpleSchemaBase}[${number | ""}]` ? _SimpleSchemaTypeMap["Output"][PT][] : never;
    type DecodeInputTuple<T extends SchemaOrCompiledSchema[], Collector extends unknown[] = []> = any[] extends T ? T extends (infer ST extends SchemaOrCompiledSchema)[] ? Data.Input<ST>[] : never : T extends [
        infer Schm extends SchemaOrCompiledSchema,
        ...infer Rest extends SchemaOrCompiledSchema[]
    ] ? DecodeInputTuple<Rest, [...Collector, Data.Input<Schm>]> : Collector;
    type DecodeOutputTuple<T extends SchemaOrCompiledSchema[], Collector extends unknown[] = []> = any[] extends T ? T extends (infer ST extends SchemaOrCompiledSchema)[] ? Data.Output<ST>[] : never : T extends [
        infer Schm extends SchemaOrCompiledSchema,
        ...infer Rest extends SchemaOrCompiledSchema[]
    ] ? DecodeOutputTuple<Rest, [...Collector, Data.Output<Schm>]> : Collector;
    ...
    export type ExtractSchema<T extends Schema | CompiledSchema> = T extends Schema ? T : T extends CompiledSchema<infer Schm> ? Schm : never;
    export type Input<SchmOrCSchm extends SchemaOrCompiledSchema> = ExtractSchema<SchmOrCSchm> extends infer Schm ? Schm extends NullSchema ? null : Schm extends OptionalSchema<infer T> ? NonNullable<Data.Input<T>> | null : Schm extends SimpleSchema ? DecodeInputSimple<Schm> : Schm extends BitpackSchema<infer T> ? ArrayOf<boolean, T> : Schm extends TupleSchema ? DecodeInputTuple<Schm> : Schm extends ObjectSchema ? DecodeInputObject<Schm> : Schm extends CustomSchemaHandler<infer InputType, infer _OutputType> ? InputType : Schm extends CompiledSchema<infer S> ? Data.Input<S> : never : never;
    export type Output<SchmOrCSchm extends Schema | CompiledSchema> = ExtractSchema<SchmOrCSchm> extends infer Schm ? Schm extends NullSchema ? null : Schm extends OptionalSchema<infer T> ? NonNullable<Data.Output<T>> | null : Schm extends SimpleSchema ? DecodeOutputSimple<Schm> : Schm extends BitpackSchema<infer T> ? ArrayOf<boolean, T> : Schm extends TupleSchema ? DecodeOutputTuple<Schm> : Schm extends ObjectSchema ? DecodeOutputObject<Schm> : Schm extends CustomSchemaHandler<infer _InputType, infer OutputType> ? OutputType : Schm extends CompiledSchema<infer S> ? Data.Output<S> : never : never;
    export {};
}
export type { BitpackSchema, CompiledArraySchema, CompiledBitpackSchema, CompiledCustomSchema, CompiledNullSchema, CompiledObjectSchema, CompiledOptionalSchema, CompiledSchema, CompiledSimpleSchema, CompiledTupleSchema, CustomSchema, CustomSchemaHandler, Data, DecodeResult, NullSchema, ObjectSchema, OptionalSchema, Schema, SchemaOrCompiledSchema, SchemaType, SimpleSchema, SimpleSchemaBase, SizeRange, TupleSchema, };
