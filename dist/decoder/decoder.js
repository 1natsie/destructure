import { schema as _schema, SchemaType, } from "../schema/schema.js";
const dvMethodMap = {
    ...
    do {
        ...
        switch (current.schemaType) {
            ...
            case SchemaType.Simple: {
                switch (current.base) {
                    case "char": {
                        const byteLength = current.byteLength *
                            (current.count === -1 ? view.getUint32(state.offset, true) : current.count);
                        current.count === -1 && (state.offset += 4);
                        const str = String.fromCharCode(...buffer.slice(state.offset, state.offset + byteLength));
                    ...
                    case "f64": {
                        ...
                        const values = new Array(current.count === -1 ? view.getUint32(state.offset, true) : current.count);
                        current.count === -1 && (state.offset += 4);
            ...
            case SchemaType.Object: {
                ...
                state.processingQueue.push(...current.entries
                    .map((entry) => ["object", value, entry[0]])
                    .reverse());
                break;
            }
